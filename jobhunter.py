...
def connect_to_sql():
    conn = mysql.connector.connect(user='root', password='root',
                                   host='127.0.0.1', port='8889', database='jobhunter', autocommit=True)
    return conn
...
def create_tables(cursor):
    ...
    company varchar (150), Created_at DATE, URL varchar(500), Title TEXT CHARSET utf8, Description TEXT CHARSET utf8); ''')
...
def check_if_job_exists(cursor, jobdetails):
    ##Add your code here
    query = f"SELECT * FROM jobs WHERE job_id = '{jobdetails['id']}'"
    return query_sql(cursor, query)

# Deletes job
def delete_job(cursor, jobdetails):
    ##Add your code here
    query = f"DELETE FROM jobs WHERE job_id = '{jobdetails['id']}'"
    return query_sql(cursor, query)
...
def add_or_delete_job(jobpage, cursor):
    ...
    print("Loading job page parsing...")
    for jobdetails in jobpage['jobs']:  # EXTRACTS EACH JOB FROM THE JOB LIST. It errored out until I specified jobs. This is because it needs to look at the jobs dictionary from the API. https://careerkarma.com/blog/python-typeerror-int-object-is-not-iterable/
        ...
        print(f"Checking database for {jobdetails['id']} and {jobdetails['title']}.")
        ...
        if is_job_found:
            print(f"A match was found!: {jobdetails['title']} from the company {jobdetails['company_name']}")

        else:
            print(f"A new job was found!: {jobdetails['title']} under the company {jobdetails['company_name']},")
            add_new_job(cursor, jobdetails)
            print(f"The new job found has been added to the database under the ID number #{jobdetails['id']}!")
            # INSERT JOB
